...
const chekOut = async (req, res) => {
  try {
    ...
    for (let j = 0; j < product.length; j++) {
      ...
      orginalPrice += (product[j].regularPrice*user.cart[j].quantity)
    }
    ...
    res.render("chekOut", { user, product, sum, offer, coupon ,orginalPrice });
  } catch (error) {
  ...
  try {
    ...
    for (const orderedProduct of orderProducts) {
      ...
      if (product) {
        ...
        const newSold = product.sold + orderedProduct.quantity;
        product.sold = Math.max(newSold, 0);
        await product.save();
      }
    }
  }
...
const buynowPlaceOrder = async (req, res) => {
  try {
    ...
    if (payment === 'cod') {
      ...
      if (product) {
        const newQuantity = product.quantity - productDetails.quantity;
        product.quantity = Math.max(newQuantity, 0);
        const newSold = product.sold + productDetails.quantity;
        product.sold = Math.max(newSold, 0);
        await product.save();
      }
      }
...
const updateOrderStatus = async (orderId, status) => {
  try {
    ...
    const orderProducts = updatedOrder.product.map(product => product);

    console.log("updateOrderStatus:"+orderProducts);

    if(updatedOrder.status == "conformed"){
    for (const orderedProduct of orderProducts) {
      const product = await Product.findById(orderedProduct.ProductId);
      if (product) {
        const newQuantity = product.quantity - orderedProduct.quantity;
    ...
    aggregationPipeline.push(
      {
        $project: {
          ...
          discount: { $round: { $abs: {$subtract: ["$regularPrice", "$totalPrice"] }}},
...
// Sales report limited in dates________________________
...
// Generate sales report in pdf________________________
...
// Generate sales report in csv________________________
